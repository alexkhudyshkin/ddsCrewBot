...
/minus - отказаться от обеда сегодня
/dinner - показать время обеда
/maxvote - показать максимально возможный голос за обед
...
/admin_minus @Username - указать пользователя, который сегодня не обедает
/admin_reset_vote - сбросить все голоса за обед и минусы
...
/settings_election_end_hour HH - час окончания голосования за обед
...
# ошибка отписки если не подписан
unsubscribe_err_msg = '''Ты не подписан на этот чат.'''
...
err_vote_msg = '''Ты не можешь участвовать в голосовании за время обеда :(
Для этого ты должен подписаться на /all
Ты можешь сделать это командой /subscribe'''

# сообщение об ошибке когда минисанувший пытается проголосовать
err_minus_vote_msg = '''Ты уже минусовал сегодня, а значит не можешь участвовать в голосовании.'''

# сообщение об ошибке при превышении допустимого времени голосования
err_vote_limit = '''Ты не можешь ставить такой голос сегодня :(
Доступное время голосования: {} минут.'''

# ошибка при минусовании другого пользователя от админа
admin_minus_error_text = '''Неверный формат команды, требуется указать только username пользователя через @user.'''

# уведомление когда минусующий пользователь успел проголосовать до минуса
vote_before_minus_text = '''Принято! Ваш голос был вычтен из времени обеда, сегодня обедаем в: <b>{}</b>.'''

# ошибка когда юзер не найден по юзернейму
err_username_not_found_text = '''Извините, такого пользователя нет в базе. Возможно он недавно сменил никнейм и ему нужно переподписаться!'''

# уведомление о невозможности принять участие в голосовании
vote_after_subscribe_text = '''К сожалению сегодня вы не сможете принять участие в голосовании, так как оно уже началось, но вы можете попросить админа сбросить голосование командой /admin_reset_vote.'''
...
too_late_err = '''Слишком поздно для участия в голосовании за время обеда!'''
no_dinner_today_err = '''Сегодня обеда нет!'''
big_end_error = '''Час окончания голосования за обед должен быть меньше часа времени обеда по умолчанию.'''
no_subscribers_err = '''Чтобы использовать эту команду, сперва нужно подписаться на чат командой /subscribe.'''
...
dinner_cnt = {} # кол-во обедающих пользователей, в начале дня считать в каждом чате и потом добавлять/убавлять в течение дня
users_penalty_cnt = {} # кол-во пользователей со штрафами считать в начале дня
penalty_sum = {} # сумма штрафов считать в начале дня
penalty = {} # словарь штрафов пользователей
votemax = {} # максимальное время голосования для пользователей без штрафов в чате
votemax_with_penalty = {} # максимальное время голосования для пользователей со штрафами в чате
users_minus = {} # список username которые минусанули в чате

# список чатов, чьи сообщения бот читает
subscribed_chats = []
# список чатов которые обедают
chat_voters = []
...
# текст оповещения пользователей о максимальном времени обеда
maxvote_text = '''<b>{}</b>: {} минут\n'''
maxvote_minus_text ='''<b>{}</b>\n'''

# сообщение об удалении сегодняшних голосов
admin_reset_vote_text = '''Голоса за обед удалены, текущее время обеда <b>{}</b>.'''
...
# функция преобразования списка обедающих чатов
def chat_voters_transform(update):
    chat_voters.clear()
    for i in update:
        chat_voters.append(i[0])
